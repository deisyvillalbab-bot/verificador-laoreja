...
ESTILO LAOREJAROJA: directo, sin amarillismo, sin estructuras "no es X es Y", distingue hechos de opiniones.
...
RESPONDE SOLO con JSON válido, sin texto extra ni backticks:
{
  ...
  "senales_alerta": "<patrones de desinformacion detectados, o vacio>"
}`;
...
exports.handler = async (event) => {
  ...
  let body;
  try { body = JSON.parse(event.body || "{}"); } catch (e) { return { statusCode: 400, headers, body: JSON.stringify({ error: "Body invalido" }) }; }

  const { noticia } = body;
  if (!noticia || !noticia.trim()) return { statusCode: 400, headers, body: JSON.stringify({ error: "Falta la afirmacion" }) };

  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return { statusCode: 500, headers, body: JSON.stringify({ error: "API key no configurada" }) };

  let messages = [{ role: "user", content: `Verifica esta afirmacion para LaOrejaRoja: "${noticia.trim()}"` }];
  ...
  return { statusCode: 500, headers, body: JSON.stringify({ error: "Demasiados turnos sin respuesta" }) };
};
